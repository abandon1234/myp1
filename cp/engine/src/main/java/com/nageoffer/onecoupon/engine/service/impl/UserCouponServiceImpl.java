...
public class UserCouponServiceImpl implements UserCouponService {
    ...
    public void redeemUserCoupon(CouponTemplateRedeemReqDTO requestParam) {
        ...
        long extractSecondField = StockDecrementReturnCombinedUtil.extractSecondField(stockDecrementLuaResult);//0 代表请求成功，1 代表优惠券已被领取完啦，2 代表用户已经达到领取上限
        transactionTemplate.executeWithoutResult(status -> {
            try {
                ...
                if (StrUtil.equals(userCouponListSaveCacheType, "direct")) {
                    ...
                    Double scored;//写后查询（Write-then-Read）策略
                    try {
                        ...
                        if (scored == null) {//如果查询过程抛出异常（如 Redis 宕机）
    ...
    public void createPaymentRecord(CouponCreatePaymentReqDTO requestParam) {
        ...
        try {
            ...
            // 验证优惠券是否正在使用或者已经被使用
            if (couponSettlementMapper.selectOne(queryWrapper) != null) {//结算表
            ...
            if (couponTemplate.getTarget().equals(0)) {
                ...
                BigDecimal maximumDiscountAmount = consumeRule.getBigDecimal("maximumDiscountAmount");// 最大优惠金额
                ...
                BigDecimal termsOfUse = consumeRule.getBigDecimal("termsOfUse");//使用门槛金额，即订单需要达到的最低金额才能使用该优惠券
                ...
                switch (couponTemplate.getType()) {
                    ...
                    case 1: // 满减券
                        ...
                        break;
                    case 2: // 折扣券
                        BigDecimal discountRate = consumeRule.getBigDecimal("discountRate");//从优惠券消费规则中提取 discountRate 折扣率(如0.9表示9折)
                        discountAmount = requestParam.getOrderAmount().multiply(discountRate);//订单金额 × 折扣率 = 实际折扣金额
                        if (discountAmount.compareTo(maximumDiscountAmount) >= 0) {//如果计算出的折扣金额超过 maximumDiscountAmount(最大优惠金额)，则以最大优惠金额为准
                            discountAmount = maximumDiscountAmount;
                        }
                        break;
                    default:
            ...
            // 计算折扣后金额并进行检查
            BigDecimal actualPayableAmount = requestParam.getOrderAmount().subtract(discountAmount);//订单金额 - 折扣金额 = 折扣后金额
