...
/**
 * 用户相关配置类
 * 流程：实现 WebMvcConfigurer 接口：通过重写其addInterceptors方法，用于向 Spring MVC 注册自定义拦截器，实现对请求的拦截处理。
 *      自定义拦截器实现HandlerInterceptor接口，重写核心方法preHandle()、postHandle()、afterCompletion()实现拦截前后业务功能
 */
@Configuration
