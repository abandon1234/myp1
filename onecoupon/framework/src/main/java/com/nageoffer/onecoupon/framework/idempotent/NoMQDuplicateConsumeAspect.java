...
public final class NoMQDuplicateConsumeAspect {
    ...
    public Object noMQRepeatConsume(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        //改进：
//        // 在检测到key已存在时，增加对CONSUMING状态的特殊处理
//        if (Objects.nonNull(absentAndGet)) {
//            if (IdempotentMQConsumeStatusEnum.CONSUMING.getCode().equals(absentAndGet)) {
//                // 如果是CONSUMING状态，可能是上次处理中断，允许重新消费
//                log.warn("[{}] Previous consumption interrupted, allowing retry.", uniqueKey);
//                // 继续执行业务逻辑而不是直接返回
//            } else {
//                // CONSUMED状态，正常拒绝重复消费
//                return null;
